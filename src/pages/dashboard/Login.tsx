...
import { adminGoogleLogin, getAdminUser } from "@/lib/store";
...
export default function DashboardLogin() {
  ...
  useEffect(() => {
    ...
    getAdminUser().then((user) => {
      if (live && user) navigate("/dashboard", { replace: true });
    });
    return () => {
      live = false;
    };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err?.code === "auth/unauthorized-domain" || err?.message?.includes("unauthorized-domain")) {
        setError("هذا النطاق غير مصرّح له في إعدادات Firebase. برجاء تسجيل الدخول من النطاق الرسمي للموقع.");
      } else if (err?.code === "auth/popup-blocked" || err?.code === "auth/popup-closed-by-user") {
        setError("تم حظر أو إغلاق نافذة تسجيل الدخول. برجاء السماح بالنوافذ المنبثقة والمحاولة مرة أخرى.");
      } else {
